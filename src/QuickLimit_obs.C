...
void QuickLimit_obs(int xxx=0){
  ...
  for(unsigned int i=0; i<ZPmasses.size(); i++){
    ...
    double x[n_Nmass], y_theory[n_Nmass], y_exp[n_Nmass], y_obs[n_Nmass]; 
    ...
    for(unsigned int j=0; j<hnmasses.size(); j++){
      ...
      double this_limit(1.), this_limit_95_down(1.), this_limit_68_down(1.), this_limit_68_up(1.), this_limit_95_up(1.), this_limit_obs(1.);
	...
	if(a==mZP && b==mN){
	  ...
	  this_limit_obs = c_obs * r_value_time;
	}     
      }
    ...
    TGraph *Limit_for_this_mZP_obs = new TGraph(n_Nmass, x, y_obs);
    ...
    Theory_for_this_mZP->Draw("lsame");
    ...
    Limit_for_this_mZP->Draw("lsame");

    Limit_for_this_mZP_obs->SetLineColor(kBlack);
    Limit_for_this_mZP_obs->SetLineWidth(3);
    Limit_for_this_mZP_obs->Draw("lsame");
    

    TLegend *lg = new TLegend(0.7, 0.7, 0.95, 0.95);
    lg->SetBorderSize(0);
    lg->SetFillStyle(0);
    lg->AddEntry(Theory_for_this_mZP, "Theory", "l");
    lg->AddEntry(Limit_for_this_mZP_obs, "Observed", "l");
    lg->AddEntry(Limit_for_this_mZP, "Expected", "l");
  ...
  for(unsigned int i=0; i<Nmasses.size(); i++){
    ...
    double x[n_ZPmass], y_theory[n_ZPmass], y_exp[n_ZPmass], y_obs[n_ZPmass];
...
*/
    ...
    TGraph *Limit_for_this_mN_obs = new TGraph(n_ZPmass,x,y_obs);
...
*/
    ...
    Limit_for_this_mN->Draw("lsame");

    Limit_for_this_mN_obs->SetLineColor(kBlack);
    ...
    lg->AddEntry(Limit_for_this_mN_obs, "Observed", "l");
    lg->AddEntry(Limit_for_this_mN, "Expected", "l");
    lg->AddEntry(Limit_for_this_mZP_68_band, "Expected #pm 1 #sigma", "lf");
    lg->AddEntry(Limit_for_this_mZP_95_band, "Expected #pm 2 #sigma", "lf");
    lg->Draw();
  ...
  while(getline(in2,elline2)){
    ...
    if(a.Atoi() > 0 && b.Atoi() > 0){
      ...
      limit_obs[mZp + "_" + mN] = c_obs * r_value_time;
  ...
  double y_exp_quarter[N_mZ_quarter], y_exp_quarter_95_down[N_mZ_quarter], y_exp_quarter_68_down[N_mZ_quarter], y_exp_quarter_68_up[N_mZ_quarter], y_exp_quarter_95_up[N_mZ_quarter], y_obs_quarter[N_mZ_quarter];
  ...
  for(int i_N = 0; i_N < N_mZ_quarter; i_N++){
    ...
    mN_low_xsec = limit_obs[current_mZp_str + "_" +current_mN_low_str];
    mN_high_xsec = limit_obs[current_mZp_str + "_" +current_mN_high_str];
    y_obs_quarter[i_N] = mN_low_xsec - 0.005 * (mN_low_xsec - mN_high_xsec) * (mN - mN_low + 0.);
  }
  ...
  TGraph *Limit_quarter_obs = new TGraph(N_mZ_quarter, x_quarter , y_obs_quarter);
 ...
 if(channel == "EE"){
  ...
  Limit_quarter->Draw("lsame");

  Limit_quarter_obs->SetLineColor(kBlack);
  ...
  lg_quarter->AddEntry(Limit_quarter_obs, "Observed", "l");
  lg_quarter->AddEntry(Limit_quarter, "Expected", "l");
  ...
  mZP_crosses.clear();
  vector<double> mZP_crosses_obs;
  vector<double> mZP_crosses_95_down, mZP_crosses_68_down, mZP_crosses_68_up, mZP_crosses_95_up;
  vector<double> mN_crosses_95_down, mN_crosses_68_down, mN_crosses, mN_crosses_68_up, mN_crosses_95_up, mN_crosses_obs;
  ...
  for(int i_a = 0; i_a < N_a; i_a++){
    ...
    double previous_diff[6] = {999., 999., 999., 999., 999., 999.};
    bool HasExclusion[6] = {false, false, false, false, false, false};
    ...
    for(int i_N = 0; i_N < N_mZp; i_N++){
      ...
      mN_low_xsec = limit_obs[current_mZp_str + "_" +current_mN_low_str];
      mN_high_xsec = limit_obs[current_mZp_str + "_" +current_mN_high_str];
      y_obs[i_N] = mN_low_xsec - 0.005 * (mN_low_xsec - mN_high_xsec) * (mN - mN_low + 0.);
      ...
      if(y_obs[i_N] < y_theory[i_N]) HasExclusion[5] = true;

      
      double current_diff[6];
      ...
      current_diff[5] = y_theory[i_N] - y_obs[i_N];
	...
	//obs
	if(current_diff[5] * previous_diff[5] < 0){
          double L1 = y_obs[i_N - 1];
          double L2 = y_obs[i_N];
          double dL = L2-L1;

          double x_meet = (L1-T1)*dZP/(dT-dL)+ZP1;
          mZP_Cross_obs.push_back(x_meet);
          mN_Cross_obs.push_back(current_a * (x_meet + 0.) + 100.0 * (1.0 - 4.0 * current_a));
        }
      ...
      previous_diff[5] = y_theory[i_N] - y_obs[i_N];
    ...
    if(mZP_Cross_obs.size()>0){
      mZP_crosses_obs.push_back(mZP_Cross_obs.at(0));
      mN_crosses_obs.push_back(mN_Cross_obs.at(0));
    }
  ...
  vector<double> limit_cross_obs_50;
  ...
  limit_cross_obs_50.clear();
  limit_cross_obs_50.push_back(limit_obs["400_150"]);
  limit_cross_obs_50.push_back(limit_obs["1600_750"]);
  limit_cross_obs_50.push_back(limit_obs["2600_1250"]);
  limit_cross_obs_50.push_back(limit_obs["2800_1350"]);
  ...
  for(unsigned int i_50 = 0; i_50 < N_Zp_50; i_50++){
    ...
    if(limit_cross_50.at(i_50) < theory_cross_50.at(i_50)) HasExclusion[2] = true;
    ...
    if(limit_cross_obs_50.at(i_50) < theory_cross_50.at(i_50)) HasExclusion[5] = true;
    ...
    current_diff[5] = theory_cross_50.at(i_50) - limit_cross_obs_50.at(i_50);
	...
	mN_Cross_68_up.push_back(0.5 * (x_meet + 0.) -50.0);
      ...
      if(current_diff[5] * previous_diff[5] < 0){
        double L1 = limit_cross_obs_50.at(i_50 - 1);
        double L2 = limit_cross_obs_50.at(i_50);
        double dL = L2-L1;

        double x_meet = (L1-T1)*dZP/(dT-dL)+ZP1;
        mZP_Cross_obs.push_back(x_meet);
        mN_Cross_obs.push_back(0.5 * (x_meet + 0.) -50.0);
      }
    }
    ...
    previous_diff[5] = theory_cross_50.at(i_50) - limit_cross_obs_50.at(i_50);
  ...
  const int n_point_obs = mZP_crosses_obs.size()+1;
  ...
  for(int i_fill = 0; i_fill < n_point_obs-2; i_fill++){
    x_mZp_obs[i_fill] = mZP_crosses_obs.at(i_fill+1);
    y_mZp_obs[i_fill] = mN_crosses_obs.at(i_fill+1);
  }
  ...
  x_mZp_obs[n_point-2] = 400.;
  y_mZp_obs[n_point-2] = 150.;
  ...
  x_mZp_obs[n_point-1] = mZP_crosses_obs.at(0);
  y_mZp_obs[n_point-1] = mN_crosses_obs.at(0);
  ...
  gr_central->SetName("Contour");
  gr_68_down->SetName("Contour_down");
  gr_68_up->SetName("Contour_up");
  gr_central->Write();
  gr_68_down->Write();
  gr_68_up->Write();
  gr_obs->SetName("Contour_obs");
  gr_obs->Write();
  gr_atlas->SetName("Contour_ATLAS");
  gr_atlas->Write();
  out->Close();
