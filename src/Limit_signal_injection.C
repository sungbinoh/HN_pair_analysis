...
void Limit_signal_injection(int xxx=1){
  ...
  TString binning = "Signal_injection_-1_evts";
  ...
  TString base_plotpath = ENV_PLOT_PATH+"/Signal_Injection_" + TString::Itoa(xxx,10) + "/" +  channel;
  ...
  TString XSEC_file = WORKING_DIR+"/output/Result_Signal_injection_" + TString::Itoa(xxx,10) + "_evts_" + channel + ".txt";
  ...
  for(unsigned int i=0; i<ZPmasses.size(); i++){
	...
	is >> e;
	is >> f;
        if(a==mZP&&mN==b) this_xsec = xsec * r_value_time;
      }
    ...
    Theory_for_this_mZP->SetLineColor(kBlack);
    Theory_for_this_mZP->SetLineWidth(3);
    Theory_for_this_mZP->SetMarkerSize(0);
    ...
    Limit_for_this_mZP->SetMarkerSize(0);
    ...
    lg->SetBorderSize(0);
    lg->SetFillStyle(0);
    lg->AddEntry(Theory_for_this_mZP, "Pseudo Observed", "lp");
...
*/
    ...
    Theory_for_this_mN->SetLineColor(kBlack);
    Theory_for_this_mN->SetLineWidth(3);
    Theory_for_this_mN->SetMarkerSize(0);
    ...
    Limit_for_this_mN->SetMarkerSize(0);
    ...
    lg->SetBorderSize(0);
    lg->SetFillStyle(0);
    lg->AddEntry(Theory_for_this_mN, "Pseudo Observed", "lp");
