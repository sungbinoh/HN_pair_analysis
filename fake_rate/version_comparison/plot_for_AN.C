...
TString DY_lowstat = "SKLowStat_DYJets";
TString DY_ptbinned = "DY_ptbinned";
...
void makehistogram(TString nameofhistogram, float xmin, float xmax, float rebin, float ymax, TString name_x, bool name_y){
  ...
  //mapfunc[name_cycle + DY_high] -> Add(GetHist(name_cycle + DY_ptbinned));
  GetHist(name_cycle + DY_high) -> Scale(0.000297135);
  mapfunc[name_cycle + DY_high] -> Add(GetHist(name_cycle + DY_low));
  //mapfunc[name_cycle + DY_high] -> Add(GetHist(name_cycle + DY_lowstat));
  ...
  for(int i = 0; i < n_kind; i++){
    ...
    //if(!samples_array[i].Contains(DY_high)) mapfunc[name_cycle + samples_array[i]] -> Scale(full_lumi);
    mapfunc[name_cycle + samples_array[i]] -> Scale(full_lumi); 
  ...
  */
  ...
  pdfname.Append("_data808_808new.pdf");
...
void plot(){
  ...
  openfile(Cycle_name, DY_ptbinned);
  openfile(Cycle_name, DY_lowstat);
  ...
  draw_histogram("m_mumu_HLT_Mu50_v", 70., 110., 1., 20000., "M(#mu#mu) (GeV)", true);
