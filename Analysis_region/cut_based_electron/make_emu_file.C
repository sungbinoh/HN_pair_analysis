...
void openfile(TString cyclename, TString samplename, TString dir_1st_tier, TString dir_2nd_tier, TString histname){
  ...
  gDirectory->cd(dir_1st_tier + "_" + dir_2nd_tier);
...
void make_hist_after_emu_method(TString nameofhistogram, TString channel){
  if(debug) cout << "[[make_hist_after_emu_method]] making " << nameofhistogram << "_" << channel << endl;
  ...
  if(debug) cout << "[[make_hist_after_emu_method]] making data driven plot of " + nameofhistogram + "_" + channel << endl;
  
  TString EMu = "_EMu";
  
  //clone emu data plot
  if(debug) cout << "[[make_hist_after_emu_method]] " + nameofhistogram + cyclename + current_data << endl;
  mapfunc[nameofhistogram + cyclename + current_data] -> Draw();
  //return;
  if(debug) cout << "returned?" << endl;
  mapfunc[nameofhistogram + "emu"] = (TH1F*)GetHist(nameofhistogram + cyclename + current_data) -> Clone(nameofhistogram);
  mapfunc[nameofhistogram + "emu"] -> Draw();
  //return;
  
  if(debug) cout << "cloned emu data hist" << endl;
  
  TString name_cycle = nameofhistogram + cyclename;
  
  //subtract non flavour symmetric bkgs
  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  WZ), -1);
  if(debug) cout << "minus WZ" << endl;
  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  ZZ), -1);
  if(debug) cout << "minus ZZ" << endl;
  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  SingleTop_s), -1);
  if(debug) cout << "minus SingleTop_s" << endl;
    
  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  SingleTbar_t), -1);
  if(debug) cout << "minus SingleTbar_t" << endl;

  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  SingleTop_t), -1);
  if(debug) cout << "minus SingleTop_t" << endl;
  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  WGtoLNuG), -1);
  if(debug) cout << "minus WGtoLNuG" << endl;
  
  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  ZGto2LG), -1);
  if(debug) cout << "minus ZGto2LG" << endl;

  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  WWW), -1);
  if(debug) cout << "minus WWW" << endl;

  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  WWZ), -1);
  if(debug) cout << "minus WWZ" << endl;

  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  WZZ), -1);
  if(debug) cout << "minus WZZ" << endl;

  mapfunc[nameofhistogram + "emu"] -> Add(GetHist(name_cycle +  ZZZ), -1);
  if(debug) cout << "minus ZZZ" << endl;
  
  mapfunc[nameofhistogram + "emu"] -> Scale(current_emu_ratio);
...
void save_hist_after_emu_method(TString dir_1st_tier, TString dir_2nd_tier, TString histname, TString channel){

  if(debug) cout << "open files for " << dir_1st_tier + "/" + dir_1st_tier + "_" + dir_2nd_tier + "/" << histname << endl;
  ...
  make_hist_after_emu_method(histname, channel);
...
void plot(){
  ...
  int n_hist = 11;
  TString histograms[] = {"h_OS_lljjjjmass",
			  "h_OS_lljjjjmass_AK8_0",
			  "h_OS_lljjjjmass_AK8_1",
			  "h_OS_lljjjjmass_AK8_2",
			  "h_OS_leadingljjmass",
			  "h_OS_secondljjmass",
			  "h_OS_llmass",
			  "h_OS_leadingLeptonPt",
			  "h_OS_secondLeptonPt",
			  "h_OS_leadingjet_pt",
			  "h_OS_secondjet_pt"
  };
  ...
  TFile *MyFile = new TFile("HN_pair_all_SKEMu_method_dilep_cat_v8-0-7.root","RECREATE");
  ...
  TString channel[] = {"_OS_DiEle", "_OS_DiMu"};
  for(int i_hist = 0; i_hist < n_hist; i_hist++){
    for(int i = 0; i < n_dir_1st; i++){
      for(int j = 0; j < 2; j++){  
	save_hist_after_emu_method(dir_1st_tier[i], "OS_EMu", histograms[i_hist] + "_" + dir_1st_tier[i] + "_OS_EMu", channel[j]);
	//return;
	origDir -> cd();
	gDirectory -> Cd(dir_1st_tier[i]);
