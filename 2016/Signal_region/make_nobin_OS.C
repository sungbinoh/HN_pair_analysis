...
void makehistogram(TString nameofhistogram, TString channel){
  ...
  bool hist_true[] = {false, false, false, false};
  ...
  if(GetHist(name_cycle + VV)){
    mapfunc[VV + hstack] -> Add(GetHist(name_cycle + VV));
    hist_true[0] = true;
  }
  if(GetHist(name_cycle + WJets)){
    mapfunc[WJets + hstack] -> Add(GetHist(name_cycle + WJets));
    hist_true[1] = true;
  }
...
void run_for_hist(TString dir, TString histname, TString dir_2nd_tier){
  ...
  openfile(Cycle_name, VV, dir, histname);
  openfile(Cycle_name, ttbar, dir, histname);
  openfile(Cycle_name, WJets, dir, histname);
...
void plot(){
  ...
  int n_hist = 8;
  TString histograms[] = {"h_leadingLeptonPt",
			  "h_leadingLeptonEta",
			  "h_secondLeptonPt",
			  "h_secondLeptonEta",
			  "h_llmass",
