...
void openfile(TString cyclename, TString samplename){
  ...
  if(samplename.Contains("data") || samplename.Contains("HNpair")) filename = cyclename + underbar + samplename + "_cat_v8-0-7.root";
  
  cout << "opening : " << filename << endl;

  if(samplename.Contains("ElEl")) mapfile[filename] = new TFile (("./signal/EE/" + filename));
  else if(samplename.Contains("MuMu")) mapfile[filename] = new TFile (("./signal/MM/" + filename));
  else mapfile[filename] = new TFile ((filename));


  
  //cout << samplename << "'s histograms are" << endl;
  TString directories[13] = {"CR1_DiEle", "CR1_DiMu", "CR1_EMu", "CR2_DiEle", "CR2_DiMu", "CR2_EMu", "CR3_DiEle", "CR3_DiMu", "CR3_EMu", "SR1_DiEle", "SR1_DiMu", "SR1_EMu", "Hists"}; 
  int N_directories = 13;
  ...
  //get signal sample's denom
  if(samplename.Contains("HNpair") ){
    double denom;
    denom = maphist["signal_eff" + cyclename + samplename] -> GetBinContent(1);
    maphist["signal_eff" + samplename] = new TH1D("denom", "denom", 1, 0., 1.);
    maphist["signal_eff" + samplename] -> Fill(0.5, denom);
  }
...
void draw_histogram(TString nameofhistogram, float xmin, float xmax, float rebin, float ymax, TString name_x, bool name_y){
  ...
  for(int i = 0; i < N_directories; i++){
    if(!directories[i].Contains("CR3_DiEle")) makehistogram(nameofhistogram + "_" + directories[i], xmin, xmax, rebin, ymax, name_x, name_y, directories[i]);
  }
...
void plot(){
  ...
  for(int i = 0; i < 8; i++){
    ...
    while(ratio > 2){
      ...
      openfile(Cycle_name, "HNpair_MuMu_WR5000_" + current_name + "_official");
      openfile(Cycle_name, "HNpair_ElEl_WR5000_" + current_name + "_official");
    }
  }
  ...
  draw_histogram("h_OS_lljjjjmass", 10., 5000., 1., 100000., "M(ll jjjj) (GeV)", true);   
  
    
  TFile *MyFile = new TFile("Data_bkg_signal.root","RECREATE");
  ...
  for(int i = 0; i < 8; i++){
    ...
    while(ratio > 2){
      TString current_name = "Zp" + TString::Itoa(Zpmass[i], 10) + "_HN" + TString::Itoa(HNmass, 10);
      cout << current_name << endl;
      TString file_name = "HNpair_MuMu_WR5000_" + current_name + "_official";
      HNmass += 100;
      ratio = (Zpmass[i] + 0.) / (HNmass + 0.);
      maphist["h_OS_lljjjjmass_SR1_DiMu" + Cycle_name + file_name] -> SetName("h_OS_lljjjjmass_SR1_DiMu_" + current_name);
      maphist["h_OS_lljjjjmass_SR1_DiMu" + Cycle_name + file_name] -> Write();
    }
  }
  ...
  for(int i = 0; i < 8; i++){
    ...
    while(ratio > 2){
      ...
      HNmass += 100;
      ratio = (Zpmass[i] + 0.) / (HNmass + 0.);
      maphist["signal_effHNpair_MuMu_WR5000_" + current_name + "_official"] -> SetName("Den_" + current_name + "_h_OS_lljjjjmass_SR1_DiMu");
      maphist["signal_effHNpair_MuMu_WR5000_" + current_name + "_official"] -> Write();
    }
  }
  ...
  for(int i = 0; i < 8; i++){
    ...
    while(ratio > 2){
      TString current_name = "Zp" + TString::Itoa(Zpmass[i], 10) + "_HN" + TString::Itoa(HNmass, 10);
      cout << current_name << endl;
      TString file_name = "HNpair_ElEl_WR5000_" + current_name + "_official";
      HNmass += 100;
      ratio = (Zpmass[i] + 0.) / (HNmass + 0.);
      maphist["h_OS_lljjjjmass_SR1_DiEle" + Cycle_name + file_name] -> SetName("h_OS_lljjjjmass_SR1_DiEle_" + current_name);
      maphist["h_OS_lljjjjmass_SR1_DiEle" + Cycle_name + file_name] -> Write();
    }
  }
  for(int i = 0; i < 8; i++){
    int HNmass = 100;
    ...
    while(ratio > 2){
      ...
      HNmass += 100;
      ratio = (Zpmass[i] + 0.) / (HNmass + 0.);
      maphist["signal_effHNpair_ElEl_WR5000_" + current_name + "_official"] -> SetName("Den_" + current_name + "_h_OS_lljjjjmass_SR1_DiEle");
      maphist["signal_effHNpair_ElEl_WR5000_" + current_name + "_official"] -> Write();
    }
  }
