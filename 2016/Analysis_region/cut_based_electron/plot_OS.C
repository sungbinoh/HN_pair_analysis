...
void draw_histogram(TString histname){
  ...
  TString dir_1st_tier = "empty";
  TString dir_2nd_tier = "empty";
  
  for(int i_1st_dir = 0; i_1st_dir < n_dir_1st; i_1st_dir++){
    if(histname.Contains(arr_dir_1st_tier[i_1st_dir])) dir_1st_tier = arr_dir_1st_tier[i_1st_dir]; 
  }
  for(int i_channel = 0; i_channel < n_channel; i_channel++){
    if(histname.Contains(channel[i_channel])) dir_2nd_tier = dir_1st_tier + channel[i_channel];
  }

  if(debug) cout << "dir : " << dir_1st_tier << "/" << dir_2nd_tier << "/" << histname << endl;
  ...
  openfile(Cycle_name, emu_method, dir_1st_tier, dir_2nd_tier, histname);

  if(dir_1st_tier.Contains("empty") || dir_2nd_tier.Contains("empty")) return;
